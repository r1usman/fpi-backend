...
router.get("/Dashboard", Protect, async (req, res) => {
  try {
    ...
    // Recent comments
    const recentComments = await Comment.find({ instructorId: req.user._id })
      .sort({ createdAt: -1 })
      .limit(5)
...
router.get("/", Protect, async (req, res) => {
  try {
    insId = req.user._id;
    const comments = await Comment.find({ instructorId: insId })
...
router.post("/:postId", Protect, async (req, res) => {
  try {
    ...
    const { content, parentComment, instructorId } = req.body;
